...
if [ "$sites" -eq "0" ]; then 
	...
	#python /home/jrick/bin/raxml_ascbias/ascbias.py -p gene${gene}_miss${miss}_mac${mac}.REF.phy -o gene${gene}_miss${miss}_mac${mac}.REF.noInv.phy
	#rm -f gene${gene}_miss${miss}_mac${mac}.REF.noInv.phy.felsenstein
	#rm -f gene${gene}_miss${miss}_mac${mac}.REF.noInv.phy.phy.stamatakis
fi 
