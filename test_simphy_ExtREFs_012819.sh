...
#######################################
				...
				then
              		...
              		raxmlHPC-PTHREADS-AVX -T 16 -s OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}.phy -n OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}_sites${sites_ref}.REF.EXT.filtered.out -j -m ASC_GTRGAMMA --asc-corr=lewis -f a -x 223 -N 100 -p 466
...
###Here is an issue...awk ###
			...
			printf "library(ape)\nlibrary(phrynomics)\nReadSNP('OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}.NOREF.phy', fileFormat='phy', extralinestoskip=1)->fullSNPs\nRemoveInvariantSites(fullSNPs, chatty=TRUE)->fullSNPs_only\nsnps <- RemoveNonBinary(fullSNPs_only, chatty=TRUE)\nWriteSNP(snps, file='OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}.NOREF.phy',format='phylip')" > Rscript.R
        		...
        		raxmlHPC-PTHREADS-AVX -T 16 -s OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}.NOREF.phy -n OUTFILE_s${sim}_q${QUAL}_miss${miss}_maf${maf}_sites${sites_noref}.NOREF.EXT.filtered.out -j -m ASC_GTRGAMMA --asc-corr=lewis -f a -x 323 -N 100 -p 476
...
day=`date +%m%d%y`
cat s${sim}*q*miss*/*bestTree* >> /project/phylogenref/scripts/sim_results/${day}-${tree_height}-batch.trees
ls s${sim}**q*miss*/*bestTree* >> /project/phylogenref/scripts/sim_results/${day}-${tree_height}-tree.names

/project/phylogenref/scripts/astral_script_Ext_012819.sh $sim $tree_height $taxa_ref
