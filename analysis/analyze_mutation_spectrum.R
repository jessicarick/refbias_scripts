...
mut$num_SNPs<-as.numeric(mut$num_SNPs)
...
mut.combined$retained <- case_when(mut.combined$num_SNPs < 1 ~ "no",
                                   TRUE ~ "yes")

ggdensity(data=mut.combined, x="num_SNPs", color="retained", fill="retained") +
  geom_density(aes(mut.combined$num_SNPs))
