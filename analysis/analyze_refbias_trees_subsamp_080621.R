...
raxml.trees <- "092321-all-subsamp-raxml.trees"
raxml.tree.names <- "092321-all-subsamp-raxml.names"
...
results.raxml <- results.raxml %>%
  mutate(
    ...
    height = sapply(raxml.tree.names,function(x) as.character(regmatches(x, regexec('([A-Z]+)-RAxML', x))[[1]][2])),
    rep = sapply(raxml.tree.names,function(x) as.integer(regmatches(x, regexec('\\.rep([0-9]+)\\.subsamp', x))[[1]][2])),
    method = "subsamp",
    quality = sapply(raxml.tree.names,function(x) as.integer(regmatches(x, regexec('_q(.*?)\\_m', x))[[1]][2])),
  ...
  left_join(refdist,by=c("simulation","height","int")) %>%
  mutate(
...
for (i in 1:length(raxml.trees)){
  ...
  results.raxml$RF.Dist.ML[i]<- InfoRobinsonFoulds(trees.both,normalize=TRUE)[1]
...
#     rf_matrix <- RF.dist(trees.subset)
# 
...
#     plot <- biplot +
#       geom_jitter(aes(color=as.factor(c(results.raxml$int[subset],"true_tree")),shape=as.factor(c(results.raxml$method[subset],"true_tree"))),alpha=0.5,size=5,height=2,width=2)+
#       theme_bw()+
...
# 
#     plot2 <- biplot +
#       geom_jitter(aes(color=as.factor(c(results.raxml$maf[subset],"true_tree")),shape=as.factor(c(results.raxml$missing[subset],"true_tree"))),alpha=0.5,size=5,height=2,width=2)+
#       theme_bw()+
#       theme(legend.text = element_text(size=rel(1.5)),
#             legend.title = element_blank(),
#             plot.margin = unit(c(6,5.5,20,10),"points"),
...
for (h in unique(as.character(results.raxml$height))){
  ...
  class(trees.subset) <- "multiPhylo"
  rf.dist <- RF.dist(trees.subset)
  rf.pcoa <- pcoa(rf.dist)
  rf.pcoa.plot <- rf.pcoa$vectors %>%
    as_tibble() %>%
    ggplot(aes(x=Axis.1,y=Axis.2)) +
    geom_point(aes(col=factor(c(results.raxml$maf[subset],"true_tree")),
    ...
    theme(
      ...
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),
      axis.title = element_text(size=18, family="Open Sans Light"),
      axis.text = element_text(size=14),
      legend.text = element_text(size=14),
      legend.title = element_blank()
    ) +
    ggtitle(paste0("Subsampled PCoA, Simulation ",i,", ",h)) +
    geom_hline(yintercept=0,lty=2,col="gray50") +
    geom_vline(xintercept=0,lty=2,col="gray50") +
    # biplot arrows for MAF
    geom_segment(
      x = 0, y = 0,
      xend = cor(as.numeric(as.character(results.raxml$maf[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
      yend = cor(as.numeric(as.character(results.raxml$maf[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1),
    ...
    annotate(geom = "text",
             x = cor(as.numeric(as.character(results.raxml$maf[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             y = cor(as.numeric(as.character(results.raxml$maf[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             label = "MAF",
             hjust=0,vjust=0
    ) +
    ...
    geom_segment(
      x = 0, y = 0,
      xend = cor(as.numeric(as.character(results.raxml$missing[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
      yend = cor(as.numeric(as.character(results.raxml$missing[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1),
      lineend = "round", # See available arrow types in example above
      linejoin = "round",
      size = 1, 
      arrow = arrow(length = unit(0.2, "inches")),
    ...
    annotate(geom = "text",
             x = cor(as.numeric(as.character(results.raxml$missing[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             y = cor(as.numeric(as.character(results.raxml$missing[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             label = "Missing",
             hjust=1,vjust=1
    ) +
    ...
    geom_segment(
      x = 0, y = 0,
      xend = cor(as.numeric(as.factor(results.raxml$int[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
      yend = cor(as.numeric(as.factor(results.raxml$int[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1),
      lineend = "round", # See available arrow types in example above
      linejoin = "round",
      size = 1, 
      arrow = arrow(length = unit(0.2, "inches")),
    ...
    annotate(geom = "text",
             x = cor(as.numeric(as.factor(results.raxml$int[subset])), rf.pcoa$vectors[,1][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             y = cor(as.numeric(as.factor(results.raxml$int[subset])), rf.pcoa$vectors[,2][1:length(subset)]) * 0.8 * sqrt(length(subset) - 1), 
             label = "INT",
             hjust=0,vjust=0
    )
