...
m.gam.short <- lmer(std.ingroup.gamma ~ int + maf + int:maf + (1 | simulation),
...
m.gam.med <- lmer(std.gamma ~ int + maf + int:maf + (1 | simulation),
                 data = results.mod[results.mod$height == "2000000",])
sum.gam.med <- summary(m.gam.med)
r.squaredGLMM(m.gam.med)

confint.gam<-data.frame(confint(m.gam.med)[-c(1:2),])
colnames(confint.gam) <- c("minCI","maxCI")
confint.gam$var <- rownames(confint.gam)
...
m.gam.long <- lmer(std.ingroup.gamma ~ int + maf + int:maf + (1 | simulation),
                  data = results.mod[results.mod$height == "10000000",])
sum.gam.long <- summary(m.gam.long)
r.squaredGLMM(m.gam.long)

confint.gam<-data.frame(confint(m.gam.long)[-c(1:2),])
colnames(confint.gam) <- c("minCI","maxCI")
confint.gam$var <- rownames(confint.gam)
