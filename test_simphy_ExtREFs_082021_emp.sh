...
if [ "$tree_height" == "lates" ]; then
	...
	if [ "$int" == "EXT" ]; then
		...
		taxa_ref="Pnye"
		ref_genoms="${REF_PATH}/references/puncross.gapsEstimated.fasta"
	fi
else
...
#for sim in `seq $sims`; do
	...
	if [[ ! -s rand_ind_sim${sim}.txt ]]; then
		if [ "$tree_height" == "lates" ]; then
			...
			echo "/project/phylogenref/data/${tree_height}/${int}_ref/aln_SRR3140997.sorted.bam" >> rand_ind_sim${sim}.txt
		elif [ "$tree_height" == "cichlids" ]; then
...
#                if [[ "$QUAL" -eq 40 ]]; then
                    ...
                    ${REF_PATH}/calc_dxy_emp.sh OUTFILE.q${QUAL}_s${sim}.${int}.vcf $sim $tree_height $int $taxa_ref $day
...
#################################
	...
	parallel --delay 2 --jobs 2  --line-buffer --env genes --env sim --env QUAL --env miss_list --env REF_PATH --env output_dir --env day --env tree_height --env int "bash ${REF_PATH}/each_maf_emp.sh {}" ::: $maf_list ::: $miss_list
